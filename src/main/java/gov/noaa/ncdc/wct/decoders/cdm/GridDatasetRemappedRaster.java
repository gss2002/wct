...
//	private static final long NATIVE_GRID_CACHE_SIZE_LIMIT = 8000*4000;
	...
	private boolean isAlternateXYOrder = false;
...
//			System.out.print("Opening Grid... ");
			...
			// traditional: time lat lon (or time y x) where lon/x is most varying dimension
			// handle alternative: time lon lat (or time x y) where lat/y is most varying
			// strangly, the Grid objects don't reflect this alternate order, so the base
			// Variable object must be used to determine dimension order.
			
			int rawXIndex = grid.getVariable().getDimensions().indexOf(grid.getXDimension());
			int rawYIndex = grid.getVariable().getDimensions().indexOf(grid.getYDimension());
			if (rawYIndex > rawXIndex) {
				this.isAlternateXYOrder = true;
			}
			else {
				this.isAlternateXYOrder = false;
			}
				
				

			if (dataCache != null && convertedUnits != null && ! convertedUnits.startsWith("Native")) {
				this.units = convertedUnits;
			}
			...
			if (coordSys.hasTimeAxis1D()) {
				...
				this.datetime = timeAxis.getCalendarDate(timeIndex).toDate();
			}
			else {
				datetime = null;
			}
...
//					Array yAxisArray = this.grid.getCoordinateSystem().getYHorizAxis().read("0:10,0:0");
//					Array xAxisArray = this.grid.getCoordinateSystem().getXHorizAxis().read("0:0,0:10");
//					double yCellSize = Math.abs(yAxisArray.getDouble(0) - yAxisArray.getDouble(1)); 
//					double xCellSize = Math.abs(xAxisArray.getDouble(0) - xAxisArray.getDouble(1)); 
//
//					System.out.println(minX+"-"+maxX+" , "+minY+"-"+maxY);
//					System.out.println("xCellsize="+xCellSize+" , yCellsize="+yCellSize);
//					System.out.println(yAxisArray);
//					System.out.println(xAxisArray);
...
//							bbox.getWidth() + xCellSize, bbox.getHeight() + yCellSize);
					...
					// For 2-d irregularly spaced grids (projections, and custom grids) the concept of 
					//  lat/lon cell size doesn't apply since it isn't consistent, so just use bbox
					//  given from NetCDF min/max of cell centroids.  Only for regular lat/lon grids
					//  do we add 1/2 cellsize to bounds to set bounds to surround the extents of the 
					//  outer grid cells (use corners not centers).
					bounds = new Rectangle2D.Double(
			...
			if ((! doResample) && this.coordSys.isLatLon() && this.coordSys.isRegularSpatial() && checkGridCacheSizeLimit()) {
				...
				for (int i=0; i<this.raster.getWidth(); i++) {
					for (int j=0; j<this.raster.getHeight(); j++) {
						if (yFlipped) {
							if (isAlternateXYOrder) {
								this.raster.setSample(i, j, 0, getCellValue(j, i));
							}
							else {
...
//				this.height = DEFAULT_REMAPPED_GRID_HEIGHT;
			...
			for (int zy=0; zy<numYTiles; zy++) {
				for (int zx=0; zx<numXTiles; zx++) {
					...
					Range yRange = new Range(zy*grid.getYDimension().getLength()/numYTiles, ((zy+1)*grid.getYDimension().getLength()/numYTiles)-1, stride);
					Range xRange = new Range(zx*grid.getXDimension().getLength()/numXTiles, ((zx+1)*grid.getXDimension().getLength()/numXTiles)-1, stride);
					
//					GridDatatype subsetGrid = null;
//					System.out.println(grid.getName()+" "+Arrays.deepToString(grid.getDimensions().toArray()));
//					if (grid.getYDimensionIndex() < grid.getXDimensionIndex()) {
//						subsetGrid = this.grid.makeSubset(null, null, null, null, xRange, yRange);
//					}
//					else {
//						subsetGrid = this.grid.makeSubset(null, null, null, null, yRange, xRange);
//					}
					
					//            GridDatatype subsetGrid = this.grid.makeSubset(null, null, null, 1, 1, stride);


					GridDatatype subsetGrid = grid;
...
//								double value = getCellValue(col, row);
								double value = isAlternateXYOrder ? getCellValue(row, col) : getCellValue(col, row);
...
//		}
	...
	public void setDestinationConversionUnits(String convertedUnits) {
