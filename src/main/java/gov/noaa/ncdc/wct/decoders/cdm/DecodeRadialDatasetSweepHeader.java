    ...
    public void setRadialDatasetSweep(RadialDatasetSweep radialDataset) throws ParseException {
        ...
        for (Attribute att : attList) {
            if (att.getDataType() == DataType.STRING || att.getDataType() == DataType.CHAR) {
                attributeMap.put(att.getShortName(), att.getStringValue());
                logger.fine("attributeMap: put - "+att.getShortName()+"="+att.getStringValue());
            }
            else {
                attributeMap.put(att.getShortName(), att.getNumericValue().toString());
                logger.fine("attributeMap: put - "+att.getShortName()+"="+att.getNumericValue());
            }
        }
