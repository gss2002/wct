...
//			Variable v = radialDataset.getNetcdfFile().findVariable(variableName);
			...
			System.out.println(radialDataset.getTimeUnits());
			System.out.println(sweep);
			System.out.println(sweep.getTime(180));
...
//					nexradTransform.transform(srcPoints, 0, dstPoints, 0, 1);
					...
					double distance = Math.sqrt((pp.getX()*pp.getX()) + (pp.getY()*pp.getY()));
...
//						azimuth += 360;
//					}
					double azimuth = Math.toDegrees(Math.atan(pp.getX()/pp.getY())); // reverse for meteorlogical coord system
					if (pp.getY() < 0) {
						azimuth += 180;
					}
					if (pp.getY() >= 0 && pp.getX() <= 0) {
						azimuth += 360;
					}
