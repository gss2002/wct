...
public class DecodeL3Header implements NexradHeader {
    ...
    private HashMap<String, String> ftmHashMap;
    ...
    public void decodeHeader(ucar.unidata.io.RandomAccessFile raf) throws DecodeException {
        ...
        try {
            ...
            //  get uncompressed NIDS data using decoder adapted from NetCDF library      
...
//            SDUS24 KLIX 292353\r\nN1RLIX
            ...
            // Special for the FTM (Free Text Message)
            else if (wmoString.substring(0, 4).equals("NOUS")) {
                ftmHashMap = processFTM(wmoString);  
                return;
            }
            else {
...
//                pcode == 165      // DHC
            )
        {
            return L3RADIAL;
                ...
                pcode == 65 ||
                pcode == 66 || 
                pcode == NexradHeader.L3PC_LOW_LAYER_COMP_REFLECTIVITY_AP_REMOVED || 
                pcode == 90 || 
                pcode == 36 ||
                pcode == 37 ||
                pcode == 38) {
...
//        }
    ...
    /**
     * Gets a String that represent a text presentation of the message data.  
     * Only applicable for the GSM (General Status Message) and FTM (Free Text Message) products.
     */    
    public String getMessageDisplayString(String filename) {
    	if (pcode == L3PC_FTM) {
    		return getFTMDisplayString(filename);
    	}
    	else if (pcode == L3PC_GSM) {
    		return getGSMDisplayString(filename);
    	}
    	else {
    		return "N/A";
    	}
    ...
    private HashMap<String, String> processFTM(String wmoString) throws IOException {

        // set product code to FTM
        pcode = L3PC_FTM;
...
//        }
        ...
        try {
        	...
        	// InMemoryRandomAccessFile has some issue and throws exception at the end of the file
        }
    ...
    /**
     * Gets the HashMap which represents the key-value pairs for the FTM data.  
     * Only applicable for the FTM (Free Text Message) products.
     */
    public HashMap<String, String> getFTMHashMap() {
        return ftmHashMap;
    }
     ...
     * Only applicable for the FTM (Free Text Message) products.
     */    
    private String getFTMDisplayString(String filename) {

        StringBuffer sb = new StringBuffer();
        sb.append("  NEXRAD Level-III FREE TEXT MESSAGE\n");
        sb.append("  FILE: " + filename+"\n");
        sb.append("");
        sb.append("  "+ftmHashMap.get("message")+"\n");        
        return sb.toString();
    ...
    private String getGSMDisplayString(String filename) {
