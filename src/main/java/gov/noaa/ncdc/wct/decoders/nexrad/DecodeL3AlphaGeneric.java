...
public class DecodeL3AlphaGeneric {
    ...
    private ArrayList<PacketCodeListener> packetListeners = new ArrayList<PacketCodeListener>();
...
//
            ...
            for (PacketCodeListener pcl : packetListeners) {
            	pcl.finishPacket();
            }
    ...
    private int readPacket(short packetCode, StringBuffer textBuffer) throws IOException, DecodeException {
        ...
        System.out.println("..reading packet code: "+packetCode +" / "+packetCodeHex);
        ...
        if (packetCode == 8) {
            ...
            for (PacketCodeListener pcl : packetListeners) {
            	pcl.processPacket(packetCode, packetBlockLen+","+iStart+","+jStart+","+valueOfTextString+","+new String(text));
            }
        ...
        // special graphic symbol
            ...
            for (PacketCodeListener pcl : packetListeners) {
            	pcl.processPacket(packetCode, packetBlockLen+","+iPos+","+jPos+","+pointFeatureType+","+pointFeatureAttribute);
            }
            ...
            for (PacketCodeListener pcl : packetListeners) {
            	pcl.processPacket(packetCode, packetBlockLen+","+iStart+","+jStart+","+new String(text));
            }
        	...
        	short colorValueIndicator = dis.readShort();
        	short contourValue = dis.readShort();

        	for (PacketCodeListener pcl : packetListeners) {
            	pcl.processPacket(packetCodeHex, colorValueIndicator+","+contourValue);
            }

        	logger.info("contour: colorValue="+colorValueIndicator+"  contourValue="+contourValue);
        }
            ...
            for (PacketCodeListener pcl : packetListeners) {
            	pcl.processPacket(packetCodeHex, iStart+","+jStart+","+lenOfVectors);
            }

            // n+8 is because 4 shorts = 8 bytes and lenOfVectors is a byte count
            for (int n=0; n<lenOfVectors; n=n+8) {
                ...
                for (PacketCodeListener pcl : packetListeners) {
                	pcl.processPacket(packetCodeHex, i1+","+j1+","+i2+","+j2);
                }
...
//        }
        	...
        	System.out.println("Packet code="+packetCode+" Not Supported.  Found packet code of: "+packetCode+" ---- "+ header.getDataURL());

        	logger.info("Packet code="+packetCode+" Not Supported.  Found packet code of: "+packetCode+" ---- "+ header.getDataURL());
            packetBlockLen = dis.readShort();
    ...
    public void addPacketListener(PacketCodeListener p) {
    	packetListeners.add(p);
    }
