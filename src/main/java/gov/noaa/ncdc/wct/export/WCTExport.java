...
import gov.noaa.ncdc.wct.decoders.cdm.StationPointDecoder;
...
//                   System.out.println("CLOSED INPUT FILE.  ");
    	...
    	if (dataTypeOverride != null) {
        	...
        	// double-check gridded data (such as .nc) to see if actually point or radial data
            if (currentDataType == SupportedDataType.GRIDDED) {
            	try {
    				...
    				else if (fd != null && fd.getFeatureType() == FeatureType.RADIAL) {
    					currentDataType = SupportedDataType.RADIAL;
    				}
        ...
        if (currentDataType == SupportedDataType.GOES_SATELLITE_AREA_FORMAT) {
            ...
            if (outputType == ExportFormatType.VECTOR) {
                ...
                else if (outputFormat == ExportFormat.GEOJSON) {
                	streamingExportProcess = new StreamingGeoJSONExport(file);
                }
...
//                streamingExportProcess.removeGeneralProgressListener(this);                                
        ...
        else if (currentDataType == SupportedDataType.GRIDDED) {
            //================================================================================
            // GRID EXPORT
            //================================================================================
...
//                            logger.info(Arrays.asList(new String[] { grid.getLastProcessedGridDatatype().getName() }));
                            ...
                            NetcdfFileWriter writer = NetcdfFileWriter.createNew(Version.netcdf3, file.toString());

                            CFGridWriter2.writeFile(gds, //griddataset
                                    Arrays.asList(new String[] { grid.getLastProcessedGridDatatype().getName() }), // gridlist
                                    subsetBbox, // latlon bbox
                                    null, // proj rect
                                    1, // horiz stride
                                    null, // z range              
                                    ( grid.getLastProcessedDateTime() != null) ? 
                                    		new CalendarDateRange(
                                    				CalendarDate.of(grid.getLastProcessedDateTime()), 0) : null,  // calendar date range
                                    1, // time stride
                                    false, // add lat/lon
                                    writer);
                            gds.close();
...
//                            throw new WCTExportException("For lat/lon datasets, currently only Gridded NetCDF export is supported");
        ...
        else if (currentDataType == SupportedDataType.POINT_TIMESERIES) {
//            throw new WCTExportException("This is POINT_TIMESERIES data type or format: " + dataURL);
        	if (outputFormat == ExportFormat.GEOJSON || outputFormat == ExportFormat.SHAPEFILE || outputFormat == ExportFormat.WKT) {
        		

        		StreamingProcess exportProcess = null;
        		if (outputFormat == ExportFormat.GEOJSON) {
        			exportProcess = new StreamingGeoJSONExport(file);
        		}
        		else if (outputFormat == ExportFormat.SHAPEFILE) {
        		...
        		StationPointDecoder decoder = new StationPointDecoder();
                decoder.addDataDecodeListener(this);
        		decoder.setSource(dataURL.toString());
        		decoder.decodeData(new StreamingProcess[] { exportProcess });
                decoder.removeDataDecodeListener(this);
        
        		
        	}
        	else if (outputFormat == ExportFormat.CSV) {
        		BufferedWriter bw = new BufferedWriter(new FileWriter(file));
        		StationTimeseriesExport tsExport = new StationTimeseriesExport(bw);

        		tsExport.addDataDecodeListener(this);
        		tsExport.process(dataURL);
        		tsExport.removeDataDecodeListener(this);
...
//                        rasterizer.setSize(new Dimension(exportGridSize, exportGridSize));
            ...
            else if (currentDataType == SupportedDataType.NEXRAD_LEVEL3 ||
                    currentDataType == SupportedDataType.NEXRAD_LEVEL3_NWS) {
                // Lazy object creation
...
//              }
            ...
            else if (currentDataType == SupportedDataType.NEXRAD_XMRG) {
                // Lazy object creation
                if (xmrgHeader == null) {
...
//              level3Decoder.decodeData();
            ...
            //================================================================================
                ...
                if (pcode == 58) {
                    ...
                    alphaDecoder = new DecodeMeltingLayer(level3Header);
                }
                else {
                ...
                if (outputType == ExportFormatType.RASTER) {
                    ...
                    throw new WCTExportException("Product Overlay products may not be exported in raster format");
                }
...
//                logger.fine("Writing: " + file);
                ...
                if (outputFormat == ExportFormat.SHAPEFILE) {
                    ...
                    NexradAlphaExport.saveGeoJSON(file, alphaDecoder);
                }
                else if (outputFormat == ExportFormat.CSV) {
...
//              xmrgDecoder.decodeData();
            ...
            else if (header.getProductType() == NexradHeader.L3MSG) {
                // Save GSM and FTM products to a text file
                String msgString = level3Header.getMessageDisplayString(scannedFile.toString());
...
//            logger.fine("Writing: " + file);
            ...
            // 1. Write supplemental data to file+"_sup.txt"
            if (outputFormat != ExportFormat.WCT_RASTER_OBJECT_ONLY) {
            	try {
            ...
            if (outputFormat == ExportFormat.SHAPEFILE) {
            	...
            	StreamingGeoJSONExport streamingProcess = new StreamingGeoJSONExport(file);
                streamingProcess.addGeneralProgressListener(this);
                decoder.decodeData(new StreamingProcess[] { streamingProcess });
            }
            else if (outputFormat == ExportFormat.CSV) {
