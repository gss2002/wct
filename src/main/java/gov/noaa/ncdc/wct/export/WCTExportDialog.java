...
import gov.noaa.ncdc.wct.ui.StationPointProperties;
...
//    };
    ...
    static {
        ...
        exportFormatMap.put("Shapefile (All Stations, Single Time)", ExportFormat.SHAPEFILE);
        exportFormatMap.put("Shapefile", ExportFormat.SHAPEFILE);

        exportFormatMap.put("GeoJSON (Polygon)", ExportFormat.GEOJSON);
        exportFormatMap.put("GeoJSON (Point Centroid)", ExportFormat.GEOJSON);
        exportFormatMap.put("GeoJSON (All Stations, Single Time)", ExportFormat.GEOJSON);
        exportFormatMap.put("GeoJSON", ExportFormat.GEOJSON);
        ...
        exportFormatMap.put("Well-Known Text (All Stations, Single Time)", ExportFormat.SHAPEFILE);
        
        exportFormatMap.put("CSV", ExportFormat.CSV);
        exportFormatMap.put("CSV (Entire Time Series)", ExportFormat.CSV);
...
//        exportFormatMap.put("KMZ (single files)", ExportFormat.KMZ);
    }
...
//        WCTDataUtils.scan(dataUrls[0], fileScanner, true, true, viewer.getDataSelector().getSelectedDataType());
        ...
        // .nc files default to GRIDDED, but they could be POINT - check for this.  If filename pattern check already shows Radial, 
        // then don't open the file to check
        
    	// check for point dataset

		FeatureDataset fd = null;
        try {

        	SupportedDataType dataTypeOverride = (viewer.getDataSelector().getSelectedDataType() == SupportedDataType.UNKNOWN) 
        			? null : viewer.getDataSelector().getSelectedDataType();
        	URL cacheURL = WCTDataUtils.scan(dataUrls[0], fileScanner, true, true, dataTypeOverride, viewer);

        	Formatter fmter = new Formatter();
        	fd = FeatureDatasetFactoryManager.open(null, cacheURL.toString(), WCTUtils.getSharedCancelTask(), fmter);

        	if (fd != null && fd.getFeatureType().isPointFeatureType()) {
        		dataType = SupportedDataType.POINT_TIMESERIES;
        	}
			...
			// allow errors to pass through, since some Level-3 products are not readable by NetCDF-Java
			e.printStackTrace();
//        	JOptionPane.showMessageDialog(this, 
...
//                }
    ...
    private void processFormatRemover(JComboBox exportFormat) {
        ...
        if (dataType == SupportedDataType.RADIAL) {
            ...
            exportFormat.removeItem("Shapefile (All Stations, Single Time)");
            exportFormat.removeItem("Well-Known Text");
            exportFormat.removeItem("Well-Known Text (All Stations, Single Time)");
            exportFormat.removeItem("GeoJSON");
            exportFormat.removeItem("GeoJSON (All Stations, Single Time)");
            exportFormat.removeItem("CSV (Entire Time Series)");
        }
            ...
            exportFormat.removeItem("Well-Known Text");
            exportFormat.removeItem("Well-Known Text (All Stations, Single Time)");
            exportFormat.removeItem("GeoJSON (Polygon)");
            exportFormat.removeItem("GeoJSON");
            exportFormat.removeItem("GeoJSON (All Stations, Single Time)");
            exportFormat.removeItem("CSV (Entire Time Series)");
        }
        else if (dataType == SupportedDataType.POINT_TIMESERIES) {
            exportFormat.removeItem("Shapefile (Polygon)");
            ...
            exportFormat.removeItem("Well-Known Text");
            exportFormat.removeItem("GeoJSON (Polygon)");
            exportFormat.removeItem("GeoJSON (Point Centroid)");
            exportFormat.removeItem("GeoJSON");
            exportFormat.removeItem("CSV");
            exportFormat.removeItem("ESRI ASCII Grid");
            exportFormat.removeItem("ESRI Binary Grid");
            exportFormat.removeItem("GeoTIFF (8-bit Grayscale)");
            exportFormat.removeItem("GeoTIFF (32-bit Real Values)");
            exportFormat.removeItem("Gridded NetCDF 3");
...
//        	if (selectedExportFormatString.contains("(Point Centroid)"))  --nothing special right now, no varible subsetting
            panel.add(new JLabel(
                    "<html><center>This data type (Station Time-Series) does not yet support <br> variable selection.  All variables will be exported. <br><br>" +
                    "Please press 'Next'.</center></html>"
               ), "center");
        }
        else if (dataType != SupportedDataType.GRIDDED && dataType != SupportedDataType.RADIAL) {
            panel.add(new JLabel(
                    "<html><center>This data type has a single variable.<br><br>" +
                    "Please press 'Next'.</center></html>"
               ), "center");
        }
        else {
...
//        jlGridName = StyledLabelBuilder. setStyledText(styledLabel, "x{2:sp} + y{2:sp} = z{2:sp}"); 
...
//                cardLayout.next(mainPanel);
        ...
        if (dataType == SupportedDataType.RADIAL) {
            ...
            panel.add(new JLabel(
                    "<html><center>This data type (Station Time-Series) does not support <br> attribute filtering. <br><br>" +
                    "Please press 'Next'.</center></html>"
               ), "center");
        }
        else {
...
//                cardLayout.next(mainPanel);
    ...
    public WCTFilter createFilter() {
        ...
        if (dataType == SupportedDataType.RADIAL) {
        	...
        	// any special stuff here, station selection?
        }
        else {
...
//        	exporter.setExportStationFilter(createFilter());
        }
        else {
            if (dataType == SupportedDataType.GRIDDED) {
                exporter.setGridExportGridIndex(gridProps.getSelectedGridIndex());
                exporter.setGridExportZIndex(gridProps.getSelectedZIndex());
                exporter.setGridExportRuntimeIndex(gridProps.getSelectedRuntimeIndex());
...
//		try {
//			// TODO: this needs to wait until grids are colored with palette files not color arrays in colorfactory
//			ExportKmlOptions exportKmlOptions = new ExportKmlOptions();
//	        exportKmlOptions.setDrawColorMapTransparentBackgroundPattern(true);
//	        String palName = viewer.getMapSelector().getGridColorTableName();
...
//	        exportKmlOptions.setPaletteURL(paletteURL);
//	        exportKmlOptions.setAltMode(AltitudeMode.CLAMPED_TO_GROUND);
//	        exportKmlOptions.setLogoURL(WCTExport.class.getResource("/images/noaa_logo_50x50.png"));
//	        exporter.setExportKmlOptions(exportKmlOptions);
//	        
