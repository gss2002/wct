...
//                        transferProgress.setProgressBarInfo(displayFilename);
                        ...
                        try {
                        	...
                        	final WCTTransferProgressDialog finalTransferProgress = transferProgress; 
                            foxtrot.Worker.post(new foxtrot.Task() {
                                public Object run() throws Exception {                    

                                    ftp.ftpConnect(dataURL.getHost(), "anonymous", "wct.ncdc.at.noaa.gov");                 
                                    ftp.getBinaryFile(dataURL.getFile(), localTmpFile.toString(), finalTransferProgress);                    
                                    localTmpFile.renameTo(localFile);
...
//    		sbuff.append("Total="+total+" bytes   "+new Date(file.lastModified())+ " ---- " + file+" \n");
    ...
    /**
     * Returns HashMap with name, size, last modified info.
     * Returns null if the file is not in the cache.
     * @param url
     * @return
     */
    public static HashMap<String, String> cacheFileProperties(URL url) {
    	if (! isInCache(url)) {
    		return null;
    	}

    	File cacheFile = getFileInCache(url);
    	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm a");
    	HashMap<String, String> props = new HashMap<String, String>();
    	props.put("File Name", cacheFile.getName());
    	props.put("Size", FileUtils.byteCountToDisplaySize(cacheFile.length()));
    	props.put("Last Modified", sdf.format(new Date(cacheFile.lastModified())));
