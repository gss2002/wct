...
public class WCTDirectoryScanner extends DirectoryScanner {
    ...
    public ScanResults[] listFiles(String sourceType, String location)
        throws FtpException, IOException, DecodeException, JSchException, SQLException, ParseException, NumberFormatException, XPathExpressionException, SAXException, ParserConfigurationException, JDOMException {
    ...
    public ScanResults[] listFiles(String sourceType, String location, ScanResultsComparator comparator, boolean includeUnknownTypes)
        ...
        if (sourceType.equals(WCTDataSourceDB.NCDC_HAS_FTP)) {
        	// process HAS or CLASS (maybe expand to NEXT and CDO in the future)
        	// HAS: HAS010673059 == ^[A-Z]{3}*[0-9]{9}$
        	// CLASS: 1744701703 == ^[0-9]{10}$
        	if (location.matches("^[A-Z]{3}[0-9]{9}$") || location.matches("^[0-9]{9}$")) {
                scanResults = scanHASDirectory(location);
        	}
        	else if (location.matches("^[0-9]{10}$")) {
                scanResults = scanCLASSDirectory(location);        		
        	}
        	else if (location.matches("^[0-9]{8}$")) {
                scanResults = scanCLASSDirectory(location);        		
        	}
        	else {
        		throw new IOException("Order number of '"+location+"' doesn't match"
        				+ " the expected formats for NCEI HAS or CLASS orders");
        	}
        ...
        else if (sourceType.equals(WCTDataSourceDB.NOAA_BDP_AWS)) {
        	scanResults = scanAWSBucket(location, includeUnknownTypes);
        }
...
//        System.out.println(Arrays.deepToString(scanResults));
     ...
     * @throws NumberFormatException 
     */
    public ScanResults[] scanAWSBucket(String prefix, boolean includeUnknownTypes) 
    		throws JDOMException, IOException, NumberFormatException, XPathExpressionException, 
    		DecodeException, SQLException, ParseException, SAXException, ParserConfigurationException {
        event.setProgress(0.0);
        event.setStatus("Listing AWS Bucket...");
        for (int n=0; n<listeners.size(); n++) {
            listeners.get(n).started(event);
        }
        
        ListResults listResults = listAWSBucket(prefix);
    ...
    throws MalformedURLException, FtpException, IOException, DecodeException, SQLException, ParseException, NumberFormatException, XPathExpressionException, SAXException, ParserConfigurationException {
        ...
        // check for HAS order
        if (directoryURL.toString().startsWith(DirectoryScanner.NCDC_HAS_HTTP_SERVER+"/"+DirectoryScanner.NCDC_HAS_HTTP_DIRECTORY) ||
        		directoryURL.toString().startsWith("ftp://"+DirectoryScanner.NCDC_HAS_FTP_SERVER+"/"+DirectoryScanner.NCDC_HAS_FTP_DIRECTORY) ) {
        	
        	String order = null;
        	if (directoryURL.toString().startsWith("ftp")) {
        		int startIndex = directoryURL.toString().indexOf(DirectoryScanner.NCDC_HAS_FTP_DIRECTORY) + DirectoryScanner.NCDC_HAS_FTP_DIRECTORY.length() + 1;
        		int endIndex = startIndex + 12; // 12 = HAS123456789
        		order = directoryURL.toString().substring(startIndex, endIndex);
        	}
        	else {
        		int startIndex = directoryURL.toString().indexOf(DirectoryScanner.NCDC_HAS_HTTP_DIRECTORY) + DirectoryScanner.NCDC_HAS_HTTP_DIRECTORY.length() + 1;
        		int endIndex = startIndex + 12; // 12 = HAS123456789
        		order = directoryURL.toString().substring(startIndex, endIndex);
        	}
        	return scanHASDirectory(order);
        }
    ...
    public ScanResults[] scanListResults(ListResults listResults, boolean includeUnknownTypes) 
        ...
        for (int n=0; n<urlList.length; n++) {
            try {
                ...
                if (listResults.getFileInfoList() != null &&
                		listResults.getFileInfoList().length == urlList.length &&
                		listResults.getFileInfoList()[n].getType() == Type.DIRECTORY) {
                	
                	System.out.println("FOUND A DIRECTORY: "+urlList[n]);
                	
                	
                	scanResults.setDescription("DIRECTORY");
                	scanResults.setLongName("DIRECTORY");
                	scanResults.setExtension("DIRECTORY");
                }
