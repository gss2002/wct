...
public class DataSelector extends JXDialog implements ScanResultsManager, ActionListener, ListSelectionListener {
    ...
    private DefaultComboBoxModel<String> dataTypeModel = 
        new DefaultComboBoxModel<String>(new String[] { " Auto ", " GRID ", " RADIAL ", " GOES_SATELLITE ", " LEVEL-3 NEXRAD " });
    private JComboBox<String> jcomboDataType = new JComboBox<String>(dataTypeModel);
...
//    private WCTExportGUI exportGUI;
    ...
    private static final Icon folderIcon = new ImageIcon(ThreddsPanel.class.getResource("/icons/gnome-fs-directory.png"));
        ...
        public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
            ...
            if (! dataSourcePanel.getDataType().equals(WCTDataSourceDB.SINGLE_FILE) &&
            	...
            	try {
            		...
            		//            	System.out.println(scanResults.length + " , "+scanResultsCacheStatus.length +" , "+resultsList.getElementCount());
            		...
            		if (scanResults[resultsList.convertIndexToModel(index)].getLongName().equals("DIRECTORY")) {
            			renderer.setIcon(folderIcon);
            		}           
            		else if (scanResultsCacheStatus != null && index < scanResultsCacheStatus.length && 
            				scanResultsCacheStatus[resultsList.convertIndexToModel(index)]) {
...
//        super(viewer, "Data Selector", false);
    ...
    private void createGUI() {
        ...
        dataSourcePanel = new DataSourcePanel(viewer, new SubmitButtonListener(), 
...
//        });
        ...
        JMenuItem itemMathTool = new JMenuItem("Math Tool");
        ...
        JMenuItem itemPointSubsetTool = new JMenuItem("Point Subset Tool (BETA)");
        itemPointSubsetTool.setToolTipText("Display Point Subset Tool");
        itemPointSubsetTool.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
    			PointSubsetDialog mathUI = new PointSubsetDialog(viewer, finalThis);
            }
        });
        ...
        morePopupMenu.add(itemPointSubsetTool);
...
//        			dialog.add(new JLabel("This feature is currently unavailable"));
        ...
        JMenuItem itemOpenPanoply = new JMenuItem("Open in NASA Panoply Data Viewer");
        itemOpenPanoply.setToolTipText("Open this file in the NASA Panoply Tool");
        itemOpenPanoply.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
...
//                long splashStop = 0L;                
//                  aboutFrame = PanAboutFrame.findFrame(true);                  
//                  final PanAboutFrame af = aboutFrame;                  
...
//                    }                    
//                  });
//                  splashStop = System.currentTimeMillis() + 2000L;
            	...
            	PanSourcesFrame psf = null;
            	try {
            	    PanColorTableList.getList();
            	    PanOverlayList.getList();
            	    
            		psf = PanSourcesFrame.findFrame(true);

            		ImageIcon small_icon = new ImageIcon(WCTFrame.class.getResource("/images/panoply_logo_24x24.png"));
            		ImageIcon med_icon24 = new ImageIcon(WCTFrame.class.getResource("/images/panoply_logo_24x24.png"));
            		ImageIcon med_icon32 = new ImageIcon(WCTFrame.class.getResource("/images/panoply_logo_32x32.png"));
            		ImageIcon large_icon = new ImageIcon(WCTFrame.class.getResource("/images/panoply_logo_50x50.png"));
...
//            		psf.setIconImages(iconList);
            		
            		// use this listener to set the icon images for all future Panoply frames
            		Toolkit.getDefaultToolkit().addAWTEventListener(new AWTEventListener() {
						@Override
						public void eventDispatched(AWTEvent evt) {
//							System.out.println("awt event: "+evt.getSource().getClass().getName());
							if (evt.getSource().getClass().getName().startsWith("gov.nasa.giss")) {
//								System.out.println("awt event: "+evt.getSource().getClass());
								if (evt.getSource() instanceof Frame) {
									((Frame)evt.getSource()).setIconImages(iconList);
								}
							}
						}            			
            		}, AWTEvent.WINDOW_EVENT_MASK);
            		
            		String showPanoplyInfoStr = WCTProperties.getWCTProperty("showPanoplyInfo");
            		if (showPanoplyInfoStr == null || ! showPanoplyInfoStr.equals("false")) {
            			showPanoplyInfoDialog();
            		}
            		
                	URL cacheURL = WCTTransfer.getURL(getSelectedURLs()[0], false, viewer);
            		PanFileUtilities.openDataset(psf, cacheURL);
            	} catch (Exception ex) {
            		if (ex.getMessage().contains("Dataset is already open")) {
...
//            		psf.pack();
            		if (! psf.isVisible()) {
            			psf.setLocationRelativeTo(viewer);
            		}
            		psf.setVisible(true);
            		psf.setSize(psf.getSize().width + 300, psf.getSize().height);
...
//                }
        ...
        morePopupMenu.add(itemOpenPanoply);
        
        
        
        
        
        JMenuItem itemOpenToolsUI = new JMenuItem("Open Unidata ToolsUI");
        itemOpenToolsUI.setToolTipText("Open this file in the Unidata ToolsUI");
        itemOpenToolsUI.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
...
//        			Not good - doesn't load all the preferences, cache settings, http auth, etc.. correctly
//                	ToolsUI tui = new ToolsUI(prefs, viewer);
//                	tui.setVisible(true);
                	
//        			Not good - will close entire WCT application with System.exit() if closed
//                	ToolsUI.main(new String[] { getSelectedURLs()[0].toString() });
        			
        			ToolsUILauncher.launch(new String[] { });
        ...
        morePopupMenu.add(itemOpenToolsUI);
...
//        JMenuItem itemOpenPanoply2 = new JMenuItem("Open in Panoply main");
//        itemOpenPanoply2.setToolTipText("Open this file in the NASA Panoply Tool");
//        itemOpenPanoply2.addActionListener(new ActionListener() {
//            public void actionPerformed(ActionEvent e) {
//            	try {
//            		Panoply.main("-m", getSelectedURLs()[0].toString());
//            	} catch (Exception ex) {
//            		
//            			ex.printStackTrace();
//            		
...
//            }
//        });
//        morePopupMenu.add(itemOpenPanoply2);
...
//        choicePanel.add(contourButton);
        ...
        try {
			...
			for (Favorite fav : favList) {
				...
				scan.setDisplayName(fav.getDisplayString() == null ? fav.getDataURL().toString() : fav.getDisplayString());
				scan.setLongName(fav.getDisplayString() == null ? fav.getDataURL().toString() : fav.getDisplayString());
...
//        menuItem.addActionListener(e -> addFavorite());
//        listPopupMenu.add(menuItem);
//        resultsList.addMouseListener(new ListPopupListener(listPopupMenu));
        resultsList.addMouseListener(new ListPopupListener(morePopupMenu));        
        threddsPanel.getItemList().addMouseListener(new ListPopupListener(morePopupMenu));
    ...
    private void addFavorite() {
        if (getDataSourcePanel().getDataType().equals(WCTDataSourceDB.THREDDS)) {
        	...
        	for (int n : selectedIndices) {
        		...
        		favCopy.setDisplayString(fav.getDisplayString() == null ? fav.getDataURL().toString() : fav.getDisplayString());
			...
			for (int n=0; n<resultsList.getSelectedIndices().length; n++) {
				...
				if (scanResults[getSelectedIndices()[n]].getDisplayName() != null &&
						! scanResults[getSelectedIndices()[n]].getDisplayName().equals("null")) {
...
//			    			urls[0], chooser.getSelectedFile().toURI().toURL());
    ...
    public void showPanoplyInfoDialog() {
    	final JDialog panInfoDialog = new JDialog(this, "NASA Panoply Data Viewer Information", true);
		
		JButton continueButton = new JButton("Continue");
		continueButton.addActionListener(new ActionListener() {						
			@Override
			public void actionPerformed(ActionEvent evt) {
				panInfoDialog.setVisible(false);
			}
		});
		JButton panWebsiteButton = new JButton("Browse to the NASA Panoply Website");
		panWebsiteButton.addActionListener(new ActionListener() {						
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					Desktop.getDesktop().browse(new URI("http://www.giss.nasa.gov/tools/panoply/"));
				} catch (Exception e2) {
					e2.printStackTrace();
				}
			}
...
//		panel.add(new JLabel(new ImageIcon(DataSelector.class.getResource("/images/panoply-splash.png"))));
		panel.add(new JLabel(
				"<html>The NASA Panoply Data Viewer is a standalone visualization tool <br>"
				+ "developed and maintained by Dr. Robert B. Schmunk at the NASA Goddard <br>"
				+ "Institute for Space Studies (GISS).  Panoply offers several important <br>"
				+ "features for Gridded Datasets which are not present in the NOAA Weather and <br>"
				+ "Climate Toolkit, including: <br>"
				+ "  1) Publication Style maps <br>"
				+ "  2) Over 75 regional and global map projections <br>"
				+ "  3) Slice and plot latitude-vertical, longitude-vertical, or time-latitude <br>"
				+ "     arrays from larger multidimensional variables <br>"
				+ "  4) PDF map export <br><br>"
				+ "Panoply is based on many of the same underlying NetCDF libraries as <br>"
				+ "the Weather and Climate Toolkit.  The Panoply software is already <br>"
				+ "included in this distribution - there is no additional software to <br>"
				+ "download or install. <br><br>"
				+ "Please check the Panoply website for more information."
			), "p");
		
		panel.add(panWebsiteButton, "p");            		
		panel.add(dontShowAgainCheckBox, "p");
		panel.add(continueButton, "p p");
		
		panInfoDialog.getContentPane().setLayout(new BorderLayout());
		panInfoDialog.getContentPane().add(
				new JLabel(new ImageIcon(DataSelector.class.getResource("/images/panoply-splash.png"))), BorderLayout.WEST);
		panInfoDialog.getContentPane().add(panel, BorderLayout.CENTER);
		panInfoDialog.pack();
		panInfoDialog.setVisible(true);
		
		// following occurs after dialog is closed
		if (dontShowAgainCheckBox.isSelected()) {
			WCTProperties.setWCTProperty("showPanoplyInfo", "false");
		}
    }
...
//            export();
    ...
    public void valueChanged(ListSelectionEvent event) {
        if (! event.getValueIsAdjusting()) {
            ...
            if (scanResults != null &&
            		...
            		resultsList.getModel().getSize() > 0 &&
            		getSelectedIndices().length > 0 &&
            		scanResults[getSelectedIndices()[0]].getLongName().equals("DIRECTORY")) {
            	jbLoad.setText("Open");
    ...
    public void loadData() {
        ...
        if (scanResults != null &&
        		...
        		scanResults.length > 0 &&
        		getSelectedIndices().length > 0 &&
        		getSelectedIndices()[0] >= 0 &&
        		! dataSourcePanel.getDataType().equals(WCTDataSourceDB.SINGLE_FILE) &&
                ! dataSourcePanel.getDataType().equals(WCTDataSourceDB.THREDDS) &&
                ! dataSourcePanel.getDataType().equals(WCTDataSourceDB.FAVORITES) &&
        		scanResults[getSelectedIndices()[0]].getLongName().equals("DIRECTORY")) {
        	try {
        		String url = dataUrl.toString().replaceAll("ftp://anonymous:wct.ncdc.at.noaa.gov@", "ftp://");
				dataSourcePanel.setDataLocation(dataSourcePanel.getDataType(), url);
				submitListFiles();
			} catch (Exception e) {
				e.printStackTrace();
			}
...
//                	dd.process();
                ...
                if (dataSourcePanel.getDataType().equals(WCTDataSourceDB.SINGLE_FILE) ||
                    ...
                    if (WCTTransfer.isInCache(url)) {
                    	HashMap<String, String> cacheFileProps = WCTTransfer.cacheFileProperties(url);
                    	int decision = JOptionPane.showOptionDialog(this, "File found in Toolkit data cache.\n" +
                    			...
                    			"File Name: "+cacheFileProps.get("File Name")+"\n" +
                    			"Size: "+cacheFileProps.get("Size")+"\n" +
                    			"Last Modified: "+cacheFileProps.get("Last Modified")+"\n" , 
                    			"Toolkit Data Cache", 
...
//System.out.println("1: "+threddsPanel.getPreferredSize());
            ...
            // special case - if NWS FTP is selected, sort descending all the time
            ...
            ScanResults[] scanResults = (ScanResults[])foxtrot.ConcurrentWorker.post(new foxtrot.Task() {
                @Override
                public Object run() throws NumberFormatException, XPathExpressionException, FtpException, IOException, 
                DecodeException, JSchException, SQLException, ParseException, SAXException, ParserConfigurationException, JDOMException {  
            ...
            if (scanResults.length == 0 && ! dataSourcePanel.getListAllFiles() && 
            		! dataSourcePanel.getDataType().equals(WCTDataSourceDB.NOAA_BDP_AWS) &&
...
//			}
		...
		public void loadFavoritesList() {
			...
			OpenFileFilter wctfavsFilter = new OpenFileFilter("wctfavs", true, "NOAA Weather and Climate Toolkit Favorites (.wctfavs)");
