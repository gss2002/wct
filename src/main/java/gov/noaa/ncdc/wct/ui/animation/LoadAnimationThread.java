...
public class LoadAnimationThread extends Thread implements ActionListener
{
    ...
    private double overallPercentComplete = 0;
    private int currentUrlIndexProcessing = 0;
...
//                individualBar.setString("Processing Data: "+progress+" % Complete");
            	...
            	if (progress > 0 && ++cnt%100 == 0) {
            		double percent = 100.0 * WCTUtils.progressCalculator(
            				new int[] { currentUrlIndexProcessing, progress } , new int[] { urlsToLoad.length, 100 });
            		//                System.out.println(percent + ", "+ overallPercentComplete + ", "+progress);
            		progressBar.setValue((int)percent);
            		progressBar.setString("Animation Processing Progress: "+WCTUtils.DECFMT_0D00.format(percent)+
            				" % ("+currentUrlIndexProcessing+" / "+urlsToLoad.length + ")");
            		progressFrame.setTitle("("+WCTUtils.DECFMT_0D00.format(percent)+" %) Animation Progress");
            	}
            }
            @Override
...
//        JPanel progressPanel = new JPanel(new RiverLayout());
//        progressPanel.add(new JLabel("Animation Processing Progress", JLabel.CENTER), "center hfill");
//        progressPanel.add(individualBar, "hfill br");
//        progressPanel.add(progressBar, "hfill br");
//        progressPanel.add(memBar, "hfill br");
//        progressPanel.add(cancelPanel, "hfill br");
        

//        JPanel framePreviewPanel = new JPanel(new BorderLayout());
        final ImageIcon framePreviewIcon = new ImageIcon();
        final JLabel framePreviewLabel = new JLabel(framePreviewIcon);
//        framePreviewPanel.add(framePreviewLabel, BorderLayout.CENTER);
        framePreviewIcon.setImage(viewer.getViewerAWTImage());
        
//        framePreviewLabel.setBorder(BorderFactory.createLineBorder(Color.BLACK));
        
        progressFrame.getContentPane().setLayout(new RiverLayout(4, 0));
        progressFrame.getContentPane().add(new JLabel("Animation Processing Progress", JLabel.CENTER), "center hfill");
        progressFrame.getContentPane().add(framePreviewLabel, "hfill br");
//        progressFrame.getContentPane().add(progressPanel, "br left");
//      progressPanel.add(individualBar, "hfill br");
        progressFrame.getContentPane().add(progressBar, "hfill br");
        memBar.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
        progressFrame.getContentPane().add(memBar, "right");
        progressFrame.getContentPane().add(cancelPanel, "right");

//        progressPanel.setBounds(200, 200, 500, 300);
//        
//        JLayeredPane layeredPane = new JLayeredPane();
//        layeredPane.setPreferredSize(new Dimension(300, 310));
//        layeredPane.add(framePreviewPanel, new Integer(1), 1);
////        layeredPane.add(progressPanel, new Integer(0), 0);
//        progressFrame.getContentPane().setLayout(new BorderLayout());
//        progressFrame.getContentPane().add(layeredPane, BorderLayout.CENTER);
            ...
            frameHandler = new FrameHandler() {
                ...
                public void processFrame(BufferedImage frameImage, String displayName, String saveName) {
                	try {
                        ...
                        framePreviewLabel.setIcon(new ImageIcon(frameImage));
...
//                    double percent = 100 * WCTUtils.progressCalculator(
//                    		new int[] { (int)overallPercentComplete,  (int)event.getProgress() } , new int[] { 100, 100 });
//                    System.out.println(percent + ", "+ overallPercentComplete + ", "+event.getProgress());
//                    progressBar.setValue((int)percent);
//                    progressBar.setString("Animation Progress: "+WCTUtils.DECFMT_0D00.format(percent)+" %");
//                    progressFrame.setTitle("("+WCTUtils.DECFMT_0D00.format(percent)+" %) Animation Progress");
...
//        animationFrame.setLocation(25, 25);
    ...
    private void setPercentComplete(double percent) {
    	this.overallPercentComplete = percent;
    }
