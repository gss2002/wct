...
public class ViewerKmzUtilities {
    ...
    public void saveKmz() throws Exception {
        ...
        this.createShadowImages = kmzDialog.isCreateShadow();
...
//                    }
                ...
                addFrameToKMZ(kmzOut, frameName, frameName+".png", 
                        ...
                        (altMode == AltitudeMode.CLAMPED_TO_GROUND) ? false : this.createShadowImages);
...
//                        date1, date2, bounds, altMode, alt, createShadowImages, addLegendOverlays);
            	...
            	if (isColladaCapable(viewer) && isDrapeOnColladaSelected) {
            		addFrameToKMZ(kmzOut, scannedFile, scannedFile, bounds, 
            				AltitudeMode.DRAPE_ON_MODEL, this.altitude, this.createShadowImages);                     
//            		addFrameToKMZ(kmzOut, saveName, saveName+"-"+uniqueString, date1, null, bounds, AltitudeMode.DRAPE_ON_MODEL, this.altitude, createShadowImages);                     
                ...
                // Append to KML
                    ...
                    addFrameToKMZ(kmzOut, scannedFile, scannedFile, bounds, 
                    		AltitudeMode.ABSOLUTE, this.altitude, this.createShadowImages);                         
                }
                else {
                    addFrameToKMZ(kmzOut, scannedFile, scannedFile, bounds);
                }
...
//        	String label = WCTUtils.DECFMT_0D000.format(bounds.getMinX())+" , "+WCTUtils.DECFMT_0D000.format(bounds.getMinY(), );
    ...
    throws IOException, WCTException, ParseException, MismatchedDimensionException, FactoryException, TransformException {
        ...
        if (createShadowImages) {
            String shadowImageName = frameName.substring(0, frameName.length()-4) + "_shadow.png";
            BufferedImage shadowImage = darkenImage(image, 4);
        	addKMZImageEntry(shadowImageName, shadowImage, kmzOut);
        }
    ...
    private BufferedImage darkenImage(BufferedImage image, int iterations) {
    	
    	if (iterations < 1) {
    		return image;
    	}
    	ImageIcon icon = ImageUtilities.createDarkerImage(new ImageIcon(image));
    	for (int n=1; n<iterations; n++) {
    		icon = ImageUtilities.createDarkerImage(icon);
    	}
    	
    	BufferedImage darkImage = AWTImageExport.toBufferedImage(icon.getImage(), true);
    	
    	BufferedImage returnImage = new BufferedImage(
    			darkImage.getWidth(), darkImage.getHeight(), BufferedImage.TYPE_INT_ARGB);
        Graphics2D g = returnImage.createGraphics();   
//        g.setComposite(AlphaComposite.getInstance(AlphaComposite.CLEAR, 0.5f));
//        g.drawImage(darkImage, 0, 0, null);
...
//    	return returnImage;
    ...
    throws IOException, MismatchedDimensionException, FactoryException, TransformException, WCTException {
        ...
        if (createShadowImages) {
            String shadowImageName = imageName.substring(0, imageName.length()-4) + "_shadow.png";
            BufferedImage shadowImage = darkenImage(image, 4);
        	addKMZImageEntry(shadowImageName, shadowImage, kmzOut);
        }
...
//            System.out.println("kmlOverlayList timestamp:  "+kmlOverlayList.get(n).getTimestamp());
            ...
            if (createShadowImages) {
                ...
                // TODO: 1 - check with z-dim gridded datasets
                createShadowImageKML(kmlString, frameName+"_shadow", 
                		kmlTimeString, nextKmlTimeString, imageName+"_shadow", frameBounds);
...
//            System.out.println("kmlModelList timestamp:  "+kmlModelList(n).getTimestamp());
            ...
            if (createShadowImages) {
                ...
                // TODO: check with 3D radial sweeps
                String shadowImageName = imageName.substring(0, imageName.length()-4) + "_shadow.png";
                createShadowImageKML(kmlString, frameName+"_shadow", 
                		kmlTimeString, nextKmlTimeString, 
                		shadowImageName, frameBounds);
