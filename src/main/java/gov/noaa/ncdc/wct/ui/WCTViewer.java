...
import gov.noaa.ncdc.nexradiv.LayerSelector;
...
import gov.noaa.ncdc.wct.decoders.nexrad.DecodeMeltingLayer;
...
import gov.noaa.ncdc.wct.ui.SnapshotLayer.DefaultLegendType;
...
ActionListener, WindowListener, ComponentListener {
	...
	public static enum CurrentDataType { NONE, RADAR, SATELLITE, MODEL, GRIDDED, STATION_POINT };
	...
	private DefaultMapLayer mlNexradAlphaLine, mlNexradRangeRings, mlMarkers, mlPointData, mlPointDataSelection;
	...
	private FeatureCollection pointLayerFeatures = FeatureCollections.newCollection();
	private FeatureCollection pointLayerSelectedFeatures = FeatureCollections.newCollection();
...
//	private DataOrganizer dataOrganizer;
	...
	private LayerSelector mapSelect;
...
//	private NexradMath nexradMath;
	...
	private StationPointProperties stationPointProps = null;

	private Frame animationProgressFrame;

	// Frame Tools
	WCTToolBar wctToolBar;
	    final JComboBox<String> jcomboViewSelector = new JComboBox<String>(new String[] { " Standard" });
//	final JComboBox<String> jcomboViewSelector = new JComboBox<String>(new String[] { " Standard", " Four Pane" });
//	final JComboBox<String> jcomboViewSelector = new JComboBox<String>(new String[] { " Standard", " Google Earth Split",});
	...
	private JMenuItem jmiDroughtMonitor, jmiSpcStormReports, jmiNhc, jmiCDRImpactTool, jmiCDOServicesTool,
		jmiBDPAccessTool;
	...
	private JMenuItem jmiLocationSearch, jmiIsosurface, jmiScreenCapture, jmiCaptureAnim, jmiMath, jmiAttributeFilter, 
	jmiStormSummary, jmiPointSubset, jmiContourManager, jmiTimeMorphing;
...
//		System.out.println("setting help pane glasspane");
//		setGlassPane(new WCTHelpGlassPane(this));
...
//		getContentPane().add(mainPanel);
		...
		jmiOverlay = new JMenuItem("Map Layer Selector", KeyEvent.VK_O);
		...
		jmiAlphaProp = new JMenuItem("Product Overlay Prop.", KeyEvent.VK_A);
		...
		jmTools = new JMenu();
		jmiLocationSearch = new JMenuItem("Location Search", KeyEvent.VK_L);
//		jmiLocationSearch.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, ActionEvent.CTRL_MASK));
		...
		jmiTimeMorphing = new JMenuItem("Time Morphing Tool (BETA)", KeyEvent.VK_T);
//		jmiTimeMorphing.setToolTipText("");
		...
		jmiMath = new JMenuItem("Math Tool", KeyEvent.VK_M);
		...
		jmiPointSubset = new JMenuItem("Point Subset Tool (BETA)", KeyEvent.VK_P);
		jmiPointSubset.setToolTipText("Extract a timeseries of values at a specific marker point.");
		...
		jmiCDOServicesTool = new JMenuItem("Station Temp./Precip. Data Browser");
		jmiCDOServicesTool.setToolTipText("View Station Data from NCEI");
		jmiBDPAccessTool = new JMenuItem("NOAA Big Data Project - Access Tool");
		jmiBDPAccessTool.setToolTipText("NOAA Big Data Project - Access Tool");
...
//		jmiGhcn.setToolTipText("Export GHCN (Global Historical Climate Network) data tool.");
		...
		jmiLocationSearch.addActionListener(this);
		...
		jmiPointSubset.addActionListener(this);
		...
		jmiCDOServicesTool.addActionListener(this);
		jmiBDPAccessTool.addActionListener(this);
...
//		jmDataServicesSubMenu.add(jmiGhcn);
		jmDataServicesSubMenu.add(jmiCDOServicesTool);
//		jmDataServicesSubMenu.add(jmiBDPAccessTool);
...
//		jmTools.add(new JSeparator());
		jmTools.add(jmiLocationSearch);
		jmTools.add(new JSeparator());
		jmTools.add(jmiIsosurface);
		jmTools.add(jmiTimeMorphing);
		jmTools.add(jmiMath);
		jmTools.add(jmiPointSubset);
...
//				 TODO change to toggle
				showSpcStormReportsTool();
			}
		});
		JMenuItem item3 = new JMenuItem("Station Temp./Precip. Data Browser");
		item3.setToolTipText("Launch the station data finder display dialog");
...
//		servicesPopupMenu.add(item4);
		...
		appButton = new JButton(new RotatedTextIcon(RotatedTextIcon.CCW, Font.decode(null), "Apps"));
		appButton.setActionCommand("Apps");
		appButton.addActionListener(viewerEventController);
		appButton.setToolTipText("Display Popup Menu for Available Apps");
		...
		mapSelectorButton.setToolTipText("Toggle Map Layer Selector Visibility");

		snapshotButton = new JButton(new RotatedTextIcon(RotatedTextIcon.CCW, Font.decode(null), "Snapshot"));
		snapshotButton.setActionCommand("Snapshot");
		snapshotButton.addActionListener(viewerEventController);
		snapshotButton.setToolTipText("Create Snapshot Layer of Current View");
		propertiesButton = new JButton(new RotatedTextIcon(RotatedTextIcon.CCW, Font.decode(null), "Properties"));
		propertiesButton.setActionCommand("Properties");
		propertiesButton.addActionListener(viewerEventController);
		propertiesButton.setToolTipText("View Properties of Current Active Layer");
...
//		leftPanel.add(appButton);
		leftPanel.add(servicesSelectorButton, "br");
		leftPanel.add(dataSelectorButton, "br");
		leftPanel.add(mapSelectorButton, "p");
		leftPanel.add(snapshotButton, "p");
		leftPanel.add(propertiesButton, "p");
...
//		viewSelectorPanel.add(geButton);
		...
		// testing
//		JPanel propsBar = new JPanel(new RiverLayout());
//		propsBar.add(new JLabel("Properties Bar................................"), "hfill");
//		topBarPanel.add(propsBar, BorderLayout.SOUTH);
...
//		}
		...
		try {
	        ...
	        URL url = ResourceUtils.getInstance().getJarResource(
	                new URL(WCTConstants.CONFIG_JAR_URL), ResourceUtils.CONFIG_CACHE_DIR, "/config/servicesMenu.xml", null);
			servicesManager.addConfig(url);
...
//		}
		else if (source == jmiPointSubset) {
			PointSubsetDialog subsetUI = new PointSubsetDialog(this);
		}
		else if (source == jmiLocationSearch) {
...
//			dataSearchUI.setLocation(this.getX()+25, this.getY()+25);
			try {
				SearchDialog ggd = SearchDialog.getInstance(this);
				ggd.setVisible(true);
			} catch (Exception e) {
				e.printStackTrace();
			}
...
//			isosurfaceUI.setLocation(this.getX()+25, this.getY()+25);
		...
		else if (source == jmiCDOServicesTool) {
			try {
				CDOServicesUI cdoTool = CDOServicesUI.getInstance(this);
				cdoTool.setLocation(this.getX()+25, this.getY()+25);
				cdoTool.setVisible(true);
		...
		else if (source == jmiBDPAccessTool) {
			try {
//				NexradBDPAccessUI bdpTool = NexradBDPAccessUI.getInstance(this);
//				bdpTool.setVisible(true);
//				bdpTool.setLocation(this.getX()+25, this.getY()+25);
...
//		}
	...
	public void showSpcStormReportsTool() {
		try {
			if (spcStormReports == null) {
				spcStormReports = new SpcStormReportsUI(this);
			}
	...
	public void showCDOServicesTool() {
		try {
			CDOServicesUI cdoTool = CDOServicesUI.getInstance(this);
			cdoTool.setLocation(this.getX()+25, this.getY()+25);
			cdoTool.setVisible(true);
	...
	public void toggleDataSelector() {
		...
		if (dataSelect.getLocation().x == this.getX()+39 && dataSelect.getLocation().y == this.getY()+35) {
	...
	public void showMapSelector() {
		...
		if (mapSelect == null) {
			try {
				mapSelect = new LayerSelector(this);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void toggleMapSelector() {
		...
		// true toggle visibility if unmoved
		if (mapSelect.getLocation().x == this.getX()+39 && mapSelect.getLocation().y == this.getY()+35) {
		...
		try {
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				...
				if (header.getProductType() != NexradHeader.L3ALPHA && 
						...
						header.getProductType() != NexradHeader.L3MSG &&
	...
	public void loadBaseMaps() throws MalformedURLException {
		...
		try {
			...
			try {
				...
				mlPointData = new DefaultMapLayer(pointLayerFeatures, StationPointProperties.getDefaultStyle(), "STATION/POINT DATA");
				map.addLayer(2*NUM_LAYERS+2, mlPointData);
...
//		        plmark.setSize(sb.literalExpression(100));
				...
				mlPointDataSelection = new DefaultMapLayer(pointLayerSelectedFeatures, style, "STATION/POINT DATA SELECTED STATION");
				map.addLayer(2*NUM_LAYERS+3, mlPointDataSelection);
			...
			try {
				...
				map.addLayer(2*NUM_LAYERS+4, mlMarkers);
...
//						tdataset.getClass().getName().equals("ucar.nc2.dt.point.UnidataStationObsDataset")) {
				try {
					...
					if (fd != null && fd.getFeatureType().isPointFeatureType()) {
						...
						stationPointProps = StationPointProperties.getInstance(this);
						stationPointProps.setVisible(true);
						try {
							stationPointProps.process(dataUrl);						

							setIsLoading(false);
							currentDataType = CurrentDataType.STATION_POINT;
							return true;
	            	...
	            	JOptionPane.showMessageDialog(this, 
	            			"--- Connection Error ---\nPlease check your internet connection and the verify data location is correct and available.\n"+
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.GOES_SATELLITE_AREA_FORMAT) {
				try {
					...
					statusBar.setGridCoordSys(null);
					if (radialProps != null) {
					...
					if (stationPointProps != null) {
						stationPointProps.setVisible(false);
						clearStationData();
					}
					
					propertiesButton.setEnabled(false);
					propertiesButton.setActionCommand("");
					jmiGridProp.setEnabled(false);
					jmiRadialProp.setEnabled(false);
				...
				try {
					...
					propertiesButton.setActionCommand("Grid Properties");
					...
					if (resetExtent) {
						...
						if (gridDatasetRaster.getBounds().getX()+gridDatasetRaster.getBounds().getWidth() > 180
								|| Double.isNaN(gridDatasetRaster.getBounds().getX()) 
								|| Double.isNaN(gridDatasetRaster.getBounds().getY()) ) {
							setWctViewExtent(WCTUtils.adjustGeographicBounds(wctMapPane.getSize(), 
									new Rectangle2D.Double(-180, -90, 360, 180)));
						}
						else {
					...
					statusBar.setGridCoordSys(gridDatasetRaster.getLastProcessedGridCoordSystem());
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				...
				try {
					...
					radialDataset = (RadialDatasetSweep) FeatureDatasetFactoryManager.open(
							...
							dataUrl.toString(), emptyCancelTask, cdmLog);
				...
				if (radialDataset == null || radialDataset.getDataVariables() == null ||
						radialDataset.getDataVariables().size() == 0) {
					StringWriter sw = new StringWriter();
					NCdumpW.print(NetcdfFile.open(dataUrl.toString()), "", sw, null);
					logger.severe(sw.toString());

					System.out.println(cdmLog.toString());
					
					throw new DecodeException("Could not create RadialDatasetSweep datatype.\n" +
							"If this file is a supported format, the file may be corrupt.\n\n" 
//							+ "Details: "+cdmLog.toString()
							, dataUrl);
				}
			...
			if (header.getProductType() == NexradHeader.L3MSG) {

				String msgMessage = level3Header.getMessageDisplayString(scannedFile.getLastScanResult().getFileName());
				WCTTextDialog msgFrame = new WCTTextDialog(this, msgMessage, "NEXRAD Level-3 Message Product");
				msgFrame.pack();
...
//					}
					...
					if (pcode == 58) {
						...
						alpha_decoder = new DecodeMeltingLayer(level3Header);
					}
					else {
...
//					NexradLegendLabelFactory.setAlphaLegendLabels(radLegendProducer, level3Header, alpha_decoder);
					...
					map.removeLayer(mlNexradAlphaLine);
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				...
				propertiesButton.setEnabled(true);
				propertiesButton.setActionCommand("Radial Properties");
...
//				setIsLoading(false);
			...
			if (currentViewType == CurrentViewType.GEOTOOLS || 
				...
				try {
					...
					// quietly handle, shouldn't happen
				}
			}
...
//        			"Data Load Error", JOptionPane.ERROR_MESSAGE);
			...
			if (e.getMessage().startsWith("Operation canceled")) {
				...
				e.printStackTrace();
				StationPointProperties.getInstance(this).setVisible(false);
	        	JOptionPane.showMessageDialog(this, 
	        			"--- Data Load Error ---\n"+
...
//	        			"Specific Error: "+e.getMessage()+
	...
	public WCTLegendPanel getLegendPanel() {
		return keyPanel;
	}
	...
	public StationPointProperties getStationPointProperties() {
		return stationPointProps;
	}
	...
	private boolean decodeAlphaNumeric() {
		...
		try {
			...
			if (alpha_decoder.getLineFeatures() != null) {
				mlNexradAlphaLine = new DefaultMapLayer(alpha_decoder.getLineFeatures(), alphaTrackStyle);
				map.addLayer(mlNexradAlphaLine);
			}

			Style alphaStyle;

			if (alpha_decoder.getLineFeatures() != null && 
					alpha_decoder.getHeader().getProductCode() == NexradHeader.L3PC_STORM_TRACKING) {
...
//				System.out.println("steve--- zIndex: "+gridDatasetRaster.getZIndex());
				...
				mapSelect.refreshAvailableUnitConversions(gridDatasetRaster.getUnits());
...
//		System.out.println(Arrays.toString(catValues));
		...
		try {
			...
			LegendEditor legendEditor = LegendEditor.getInstance(this, this);
			if (! legendEditor.isEngageCustomLegendOverride()) {
...
//					radarGC = WCTGridCoverageSupport.applyAlpha2(radarGC, 255);
	...
	public void snapshotCurrentLayer() {
		...
		if (currentDataType == CurrentDataType.RADAR) {
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				raster = radialDatasetRaster;
			}
			else {
				raster = getWCTRasterizer();
			}
			...
			try {
				...
				snapshotLayer.setLegendProducer(radLegendProducer);
				snapshotLayer.setDefaultLegendType(DefaultLegendType.LARGE);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if ((scannedFile.getLastScanResult().getDataType() == SupportedDataType.NEXRAD_LEVEL3 ||
			...
			try {
				...
				snapshotLayer.setLegendProducer(gridSatLegendProducer);
				snapshotLayer.setDefaultLegendType(DefaultLegendType.MEDIUM);
...
//		}
		...
		snapshotLayer.setRaster(raster);
		...
		String dataName = "";
//		CategoryLegendImageProducer legendProducer = null;
		SnapshotLayer.DefaultLegendType defaultLegendType = null;
		if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.GRIDDED ||
				scannedFile.getLastScanResult().getDataType() == SupportedDataType.GOES_SATELLITE_AREA_FORMAT) {
			for (String s : lastDecodedLegendProducer.getLegendTitle()) {
				dataName += s + " ";
			}   
			defaultLegendType = DefaultLegendType.MEDIUM;
		}
		else {
			dataName = scannedFile.getLastScanResult().getLongName();
			defaultLegendType = DefaultLegendType.LARGE;
		}

		snapshotLayer.setName("Snapshot "+mapSelect.getSnapshotLayerCounter()+" ("+dataName+")");
		snapshotLayer.setLegendImage(legendImage);
		snapshotLayer.setDataURL(dataUrl);
		snapshotLayer.setDefaultLegendType(defaultLegendType);
		snapshotLayer.setLegendProducer(lastDecodedLegendProducer);
	...
	public FeatureCollection getStationPointFeatures() {
		return pointLayerFeatures;
	}
	public FeatureCollection getStationPointSelectedFeatures() {
		return pointLayerSelectedFeatures;
	}
	public DefaultMapLayer getStationPointMapLayer() {
		return mlPointData;
	}
...
//			}
	...
	public void clearStationData() {
		pointLayerFeatures.clear();
		pointLayerSelectedFeatures.clear();
	}
	...
	public void setShowAlphanumericLayers(boolean on) {
		...
		if (mlNexradAlphaLine != null) {
			mlNexradAlphaLine.setVisible(on);
		}
	...
	/**
	 * Sets the alphanumeric fill color 
	 *
	 * @param  newFillColor  
	 */
	public void setAlphanumericFillColor(Color newFillColor) {
		setAlphanumericStyling(newFillColor, this.alphaLineColor, this.alphaLineWidth, this.alphaSymbol);
	}
	...
	public void setAlphanumericLineColor(Color newColor) {
		setAlphanumericStyling(this.alphaFillColor, newColor, this.alphaLineWidth, this.alphaSymbol);
	...
	public void setAlphanumericLineWidth(int newWidth) {
		setAlphanumericStyling(this.alphaFillColor, this.alphaLineColor, newWidth, this.alphaSymbol);
	}
	 ...
	 * @param  newSymbol  
	 */
	public void setAlphanumericStyling(Color newFillColor, Color newLineColor, int newLineWidth, String newSymbol) {
		try {

			this.alphaLineColor = newLineColor;
			this.alphaFillColor = newFillColor;
			this.alphaLineWidth = newLineWidth;
			this.alphaSymbol = newSymbol;

			MapLayer ml = mlNexradAlpha;
			
			if (alpha_decoder.getFeatures() != null && alpha_decoder.getFeatures().size() > 0) {
				...
				if (Point.class.isAssignableFrom(geometryClass)
					...
					Mark mark = sb.createMark(alphaSymbol, alphaFillColor, alphaLineColor, alphaLineWidth);
			...
			if (alpha_decoder.getLineFeatures() != null && alpha_decoder.getLineFeatures().size() > 0) {
				if (alpha_decoder.getHeader().getProductCode() == NexradHeader.L3PC_STORM_TRACKING) {
					...
					try {
						...
						Color color = new Color(newFillColor.getRGB());
						...
						mlNexradAlphaLine.setStyle(sb.createStyle(sb.createLineSymbolizer(alphaLineColor, alphaLineWidth)));
				...
				else if (alpha_decoder.getHeader().getProductCode() == NexradHeader.L3PC_MELTING_LAYER) {
					...
					try {
						...
						Color linecolorBeamCenter = alphaLineColor;
						Color linecolorBeamEdge = alphaLineColor.darker();
						for (int i = 0; i < rules.length; i++) {

							filters[i] = ffi.createBetweenFilter();
							if (i==0 || i==3) {
								rules[i] = sb.createRule(sb.createLineSymbolizer(linecolorBeamEdge, alphaLineWidth));					
							}
							else {
								rules[i] = sb.createRule(sb.createLineSymbolizer(linecolorBeamCenter, alphaLineWidth*2));								
							}

							filters[i].addLeftValue(sb.literalExpression(i+1));
							filters[i].addRightValue(sb.literalExpression((i+1 + 0.99)));
							filters[i].addMiddleValue(ffi.createAttributeExpression(
									alpha_decoder.getFeatureTypes()[0], "contourVal"));
							rules[i].setFilter(filters[i]);

							alphaStyle.addFeatureTypeStyle(sb.createFeatureTypeStyle(null, rules[i]));
						}
						mlNexradAlphaLine.setStyle(alphaStyle);
...
//			}
	...
	public void setAlphanumericSymbol(String symbol) {
		...
		setAlphanumericStyling(alphaFillColor, alphaLineColor, alphaLineWidth, symbol);
...
//						sb.createStyle(sb.createLineSymbolizer(getLayerLineColor(index), newWidth)));
	...
	public BufferedImage getViewerBufferedImage() {
		...
		if (currentViewType == CurrentViewType.GOOGLE_EARTH_SPLIT_GEOTOOLS || currentViewType == CurrentViewType.GOOGLE_EARTH) {
			...
			if (stationPointProps != null) stationPointProps.setVisible(false);
...
//	}
	...
	private void saveFileAs() {
		...
		OpenFileFilter wctprojFilter = new OpenFileFilter("wctproj", true, "NOAA Weather and Climate Toolkit Files");
	...
	public LayerSelector getMapSelector() {
		if (mapSelect == null) {
			try {
				mapSelect = new LayerSelector(this);
			} catch (Exception e) {
...
//			}
	...
	private void setCurrentViewTypeValue(CurrentViewType viewType) throws BindException, Exception {
		...
		if (viewType == CurrentViewType.GOOGLE_EARTH || viewType == CurrentViewType.NCDC_NCS ||
			...
			final FourPanelMapView fpmv = new FourPanelMapView(this, true);
			mapPanel.removeAll();
			try {
				mainPanel.remove(infoPanel);
			} catch (Exception e) {};
...
//			org.apache.log4j.Logger.getLogger("net.sf.ehcache").setLevel(l4jLevel);
			...
			if (args.length >= 1 && args[0].endsWith(".wctproj")) {
				...
				dataSelect.setLocation(map.getX()+39, map.getY()+35);
				dataSelect.setVisible(true);
