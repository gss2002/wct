...
public class DataSourcePanel extends JPanel {
    ...
    private NexradBDPAccessPanel bdpPanel = null;
     ...
     */
    public DataSourcePanel(WCTViewer viewer, ActionListener submitListener, 
    			ActionListener sortByListener, KeyListener filterKeyListener, 
    			ActionListener favoritesListener) {
        this(viewer, submitListener, sortByListener, filterKeyListener, favoritesListener, true);
    }
     ...
     * @param  favoritesListener - handles events from buttons on 'favorites' tab
     */
    public DataSourcePanel(WCTViewer viewer, ActionListener submitListener, ActionListener sortByListener, 
...
//      jcomboSshServer.addItemListener(this);
        ...
        hasPanel.add("center", new JLabel("Access to NCEI HAS Order Results on NCEI FTP Site"));
        ...
        hasPanel.add("p left", new JLabel("Enter Job Number (HAS or CLASS)"));
...
//        JButton jbShowServicesMenu = new JButton("   Show Services   ");
//        jbShowServicesMenu.addActionListener(e -> {
//        	viewer.showServicesMenu();
//        });
...
//        findDataPanel.add("center", new JLabel("A guide to some popular data access sites"));
		...
		String bottomMsg = "Some direct data access from the WCT is available through the 'Services' menu accessible on the left button bar. ";
...
//        buttonPanel.add("br br", jbShowServicesMenu);
        
        findDataPanel.add(buttonPanel, BorderLayout.EAST);
        findDataPanel.add(new JXLabel(bottomMsg), BorderLayout.SOUTH);
...
//        favoritesPanel.add("left", removeAllButton);
        ...
        try {
			bdpPanel = new NexradBDPAccessPanel(viewer, submitListener);
        ...
        if (bdpPanel != null) CURRENT_TAB_ORDER_LOOKUP.put(8, WCTDataSourceDB.NOAA_BDP_AWS);
        CURRENT_TAB_ORDER_LOOKUP.put(2, WCTDataSourceDB.LOCAL_DISK);
        CURRENT_TAB_ORDER_LOOKUP.put(3, WCTDataSourceDB.URL_DIRECTORY);
        CURRENT_TAB_ORDER_LOOKUP.put(5, WCTDataSourceDB.SINGLE_FILE);
        CURRENT_TAB_ORDER_LOOKUP.put(4, WCTDataSourceDB.THREDDS);
        CURRENT_TAB_ORDER_LOOKUP.put(0, WCTDataSourceDB.NCDC_HAS_FTP);
//        CURRENT_TAB_ORDER_LOOKUP.put(1, "CLASS Order");
        CURRENT_TAB_ORDER_LOOKUP.put(6, WCTDataSourceDB.FAVORITES);
        
        
        TAB_PANEL_LOOKUP.put("Find Data", findDataPanel);
        if (bdpPanel != null) TAB_PANEL_LOOKUP.put(WCTDataSourceDB.NOAA_BDP_AWS, bdpPanel);
        TAB_PANEL_LOOKUP.put(WCTDataSourceDB.LOCAL_DISK, localPanel);
        TAB_PANEL_LOOKUP.put(WCTDataSourceDB.URL_DIRECTORY, customPanel);
        TAB_PANEL_LOOKUP.put(WCTDataSourceDB.SINGLE_FILE, sfPanel);
        TAB_PANEL_LOOKUP.put(WCTDataSourceDB.THREDDS, threddsPanel);
        TAB_PANEL_LOOKUP.put(WCTDataSourceDB.NCDC_HAS_FTP, hasPanel);
//        TAB_PANEL_LOOKUP.put("CLASS Order", classPanel);
        TAB_PANEL_LOOKUP.put(WCTDataSourceDB.FAVORITES, favoritesPanel);
...
//        }
    ...
    public String getDataType() {
    	...
    	if (selectedTabPaneName.equals(WCTDataSourceDB.NCDC_HAS_FTP)) {
    		return WCTDataSourceDB.NCDC_HAS_FTP;
    	}
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.CLASS_ORDER)) {
    		return WCTDataSourceDB.CLASS_ORDER;
    	}
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.LOCAL_DISK)) {
    		return WCTDataSourceDB.LOCAL_DISK;
    	}
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.URL_DIRECTORY)) {
    		return WCTDataSourceDB.URL_DIRECTORY;
    	}
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.THREDDS)) {
    		return WCTDataSourceDB.THREDDS;
    	}
    	//        else if (tabPane.getSelectedIndex() == 5) {
    	//            return WCTDataSourceDB.SSH_DIRECTORY;
    	//        }
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.SINGLE_FILE)) {
    		return WCTDataSourceDB.SINGLE_FILE;
    	}
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.FAVORITES)) {
    		return WCTDataSourceDB.FAVORITES;
    	}
    	else if (selectedTabPaneName.equals(WCTDataSourceDB.NOAA_BDP_AWS)) {
    		return WCTDataSourceDB.NOAA_BDP_AWS;
    	}
    	else {
    		return "UNKNOWN";
    	}
...
//        tabPanelLookup.put("CLASS Order", classPanel);
    	...
    	if (TAB_PANEL_LOOKUP.get(dataType) != null) {
    		tabPane.setSelectedComponent(TAB_PANEL_LOOKUP.get(dataType));
    	}
    	else {
    		throw new Exception("dataType: '"+dataType+"' is not supported!!!");
    	}
...
//    	if (dataType.equals(WCTDataSourceDB.NCDC_HAS_FTP)) {
//            tabPane.setSelectedComponent(TAB_PANEL_LOOKUP.get("NCEI/CLASS Order"));
//        }
//        else if (dataType.equals(WCTDataSourceDB.CLASS_ORDER)) {
...
//        }
    ...
    public String getDataLocation() {
        ...
        else if (getDataType().equals(WCTDataSourceDB.NOAA_BDP_AWS)) {
            return bdpPanel.getDataPath();
        }
