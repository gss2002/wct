...
//		System.out.println( styleInfo.getMaxScale() + " , "+ styleInfo.getMinScale());
	...
	public void loadBaseMaps(WCTViewer viewer) throws MalformedURLException {
		...
		try {
			url = ResourceUtils.getInstance().getJarResource(mapDataURL, ResourceUtils.RESOURCE_CACHE_DIR, "/shapefiles/states-natearth-360.shp", viewer);
			ShapefileDataStore ds = new ShapefileDataStore(url);
			FeatureSource fs = ds.getFeatureSource("states-natearth-360");
			...
			TextSymbolizer ts = sb.createTextSymbolizer(new Color(235, 167, 0), font, "name");
...
//			baseMapLayers.setElementAt( new DefaultMapLayer(duplicateFeatureLongitudeAdd360( fs.getFeatures().collection() ), styleFG, "_COUNTRIES"), WCTViewer.COUNTRIES_OUT);
			...
			org.geotools.styling.Font font = sb.createFont(FONT_ARRAY[3]);
			TextSymbolizer ts = sb.createTextSymbolizer(new Color(5, 167, 0), font, "name");
			ts.setHalo(sb.createHalo(Color.BLACK, 1, 2));
			Style labelStyle = sb.createStyle(ts);
...
//			
		...
		try {
			...
			url = ResourceUtils.getInstance().getJarResource(mapDataURL, ResourceUtils.RESOURCE_CACHE_DIR, "/shapefiles/major_highways2.shp", viewer);
			ShapefileDataStore dsHwyInt = new ShapefileDataStore(url);
			FeatureSource fsHwyInt = dsHwyInt.getFeatureSource("major_highways2");
...
//            BetweenFilter btwfilter = ffi.createBetweenFilter();
//            btwfilter.addLeftValue(sb.literalExpression(90));
//            btwfilter.addRightValue(sb.literalExpression(100));
//            btwfilter.addMiddleValue(ffi.createAttributeExpression(fsHwyInt.getSchema(), "ObjectID"));
//			hwyIntRule.setFilter(btwfilter);
