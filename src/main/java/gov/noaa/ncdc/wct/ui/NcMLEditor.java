...
public class NcMLEditor extends WCTFrame {
	...
	private void createGUI() {
		...
		ncmlPanel = new WCTTextPanel(ncmlString, SearchBarProperties.SEARCH_BAR_FULL);
		

		String cdlString = DataSelectionUtils.getDumpString(gov.noaa.ncdc.wct.ui.DataSelectionUtils.Type.CDL, viewer, 
				viewer.getDataSelector(), viewer.getDataSelector().getSelectedURLs()[0], true);
		WCTTextPanel cdlPanel = new WCTTextPanel(cdlString, SearchBarProperties.SEARCH_BAR_FULL);
		...
		final WCTFrame finalThis = this;
		JButton jbBrowseToNcmlDocs = new JButton("Browse NcML Documentation");
		jbBrowseToNcmlDocs.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					Desktop.getDesktop().browse(
							new URI("https://www.unidata.ucar.edu/software/thredds/current/netcdf-java/ncml/Cookbook.html"));
				} catch (IOException e1) {
					e1.printStackTrace();
	                   javax.swing.JOptionPane.showMessageDialog(finalThis, "ERROR BROWSING DOCUMENTATION (1)", 
			                     "EXPORT ERROR", javax.swing.JOptionPane.ERROR_MESSAGE);                  
				} catch (URISyntaxException e1) {
					e1.printStackTrace();
	                   javax.swing.JOptionPane.showMessageDialog(finalThis, "ERROR BROWSING DOCUMENTATION (2)", 
			                     "EXPORT ERROR", javax.swing.JOptionPane.ERROR_MESSAGE);                  
				}
			}
		});
