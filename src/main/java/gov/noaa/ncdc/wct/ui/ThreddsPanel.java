...
//            String name = path.substring(path.lastIndexOf("/")+1);
        ...
        for (int n=0; n<httpInvAccessList.size(); n++) {
            int index = directoryInvDatasetList.size() + opendapInvAccessOpendapList.size() + cnt;
...
//            }
        ...
        // don't show HTTP download option for .ncml files, which represent aggregations or virtual datasets 
        // which must be accessed via opendap
        for (int n=0; n<listModel.getSize(); n++) {
        	ThreddsEntry entry = ((ThreddsEntry)(listModel.get(n)));
        	if (entry.getName().endsWith(".ncml") && entry.getType() == ThreddsEntryType.HTTPSERVER) {
        		listModel.remove(n);
        	}
        }
