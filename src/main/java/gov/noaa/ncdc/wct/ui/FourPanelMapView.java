...
public class FourPanelMapView extends JPanel {
	...
	private int currentTargetPanelIndex = 0;
	...
	PanelRefreshListener[] panelRefreshListenerArray = new PanelRefreshListener[] {
			new PanelRefreshListener(0),
			new PanelRefreshListener(1),
			new PanelRefreshListener(2),
			new PanelRefreshListener(3)
	};
									...
									try {
										if (lastRefreshInfo[i] != null) {
											...
											if (i == getCurrentSelectedPanelIndex()) {
												refreshViewer(lastRefreshInfo[i], i);
//												updatePanel(i);
											}
		...
		for (int i=0; i<4; i++) {
			mapPanes[0].addZoomChangeListener(zoomListener);
		}
...
//				cursorMapLayer.setVisible(false);
		...
		MouseMotionListener motionListener = new MouseMotionListener() {
			...
			CoordinatePoint cpoint = new CoordinatePoint(0, 0);
...
//		        	coordPoint2d = ((GeoMouseEvent)e).getMapCoordinate(new Point2D.Double());
		        ...
		        int mpX = e.getX();
				int mpY = e.getY();
				int paneWidth = mapPanes[0].getWidth() -0; // any correction needed for border?
				int paneHeight = mapPanes[0].getHeight() -0;
		        
		        
		        double geoX = mapPanes[0].getVisibleArea().getX() + ((double)mpX/paneWidth) * mapPanes[0].getVisibleArea().getWidth();
		        double geoY = mapPanes[0].getVisibleArea().getY() + ((double)(paneHeight-mpY)/paneHeight) * mapPanes[0].getVisibleArea().getHeight();
		        coordPoint2d = new Point2D.Double(geoX, geoY);
		        coord.x = coordPoint2d.getX();
		        coord.y = coordPoint2d.getY();
		        
//				System.out.println(mpX+","+paneWidth+"  "+mapPanes[0].getVisibleArea().getX()+","+mapPanes[0].getVisibleArea().getWidth());
//				System.out.println(mpY+","+paneHeight+"  "+mapPanes[0].getVisibleArea().getY()+","+mapPanes[0].getVisibleArea().getHeight());
...
//						coordPoint2d.setLocation(coord.x, coord.y);
						cpoint.setLocation(coordPoint2d);
						 if (panelGridCoverage[i].getEnvelope().contains(cpoint)) {
							 vals = panelGridCoverage[i].evaluate(coordPoint2d, vals);			        
							 dataValues[i][2] = "["+i+"] "+WCTUtils.DECFMT_0D0000.format(vals[0])+" ";							 
						 }
						 else {
								dataValues[i][2] = "["+i+"] "+"outside envelope";
						 }
					}
					else {
						dataValues[i][2] = "["+i+"] "+"gc null";
					}
				}					
		...
		for (int n=0; n<4; n++) {
			mapPanes[n].addMouseMotionListener(motionListener);
		}
...
//		glassPane.setVisible(true);
		...
		for (int n=0; n<4; n++) {
			viewer.addRenderCompleteListener(panelRefreshListenerArray[n]);
		}
...
//		JOptionPane.showMessageDialog(this, " "+mapPanes[0].getWidth()+", "+mapPanes[0].getHeight());
	...
	public void selectPanel(int index) {
		...
		this.currentTargetPanelIndex = index;
		
//		viewer.getStatusBar().set
		
//		mapPanes[index].setBorder(BorderFactory.createMatteBorder(15, 15, 15, 15, Color.CYAN));
//		for (int n=0; n<mapPanes.length; n++) {
//			if (n != index) {
//				mapPanes[n].setBorder(BorderFactory.createLineBorder(Color.WHITE, 1));
//			}
//		}
	...
	public void updatePanel(int panelIndex) {
		...
		System.out.println("updating panel (index): "+panelIndex);
...
//        	panelUnits[panelIndex] = viewer.getRadarLegendImageProducer().get
...
//				legend[panelIndex].setInsets(new Insets(0, 0, legendBottomInset, legendImage[panelIndex].getWidth(this)-10));
	...
	public int getCurrentTargetPanelIndex() {
		return currentTargetPanelIndex;
	}
	...
	public void refreshViewer(RefreshInfo ri, int panelIndex) throws IllegalAccessException, InstantiationException {
...
//		viewer.setLoad
		...
		System.out.println("PANEL "+panelIndex+" REFRESH: LOADING: "+ri.getUrl());
...
//		viewer.addRenderCompleteListener(panelRefreshListener);
	...
	public class PanelRefreshListener implements RenderCompleteListener {
		...
		int panelIndex;
		public PanelRefreshListener(int panelIndex) {
...
//			if (currentTargetPanelIndex == panelIndex) {
				updatePanel(panelIndex);
//			}
...
////				try {
//
//					if (lastRefreshInfo[n] != null && n != currentTargetPanelIndex) {
//						System.out.println("COULD BE REFRESHING PANEL "+n+"::::::: "+lastRefreshInfo[n]);
...
//				}
//			}
//			selectPanel(currentTargetPanelIndex);
...
//				((Graphics2D)g).setColor(Color.GREEN.darker().darker()); 
//				((Graphics2D)g).setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.7f));
//				((Graphics2D)g).fillRoundRect(cursorImitatorX[n]+12, cursorImitatorY[n]-65, 70, 65, 5, 5);
				
//				if (n == currentTargetPanelIndex) {
//					Point p = SwingUtilities.convertPoint(mapPanes[n], 0, 0, viewer);	
//					((Graphics2D)g).fillRect((int)p.getX(), (int)p.getY(), mapPanes[n].getWidth(), 30);
//				}
...
//				((Graphics2D)g).drawString(dataValueString[n][2], cursorImitatorX[n]+20, cursorImitatorY[n]+10);
				for (int x=0; x<4; x++) {
//					System.out.println(dataValueString[x][2]);
					((Graphics2D)g).setColor(Color.GREEN.darker().darker()); 
					((Graphics2D)g).setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.7f));
					((Graphics2D)g).fillRoundRect(cursorImitatorX[n]-70+n%2*80, cursorImitatorY[n]-65+(n+1)%2*65, 70, 65, 5, 5);
					
					((Graphics2D)g).drawString(dataValueString[x][2], cursorImitatorX[n]+20, cursorImitatorY[n]-50+14*x);
				}
			}
