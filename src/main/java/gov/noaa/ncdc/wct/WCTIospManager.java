...
//        }
    ...
    public void registerTypedDatasetFactory(FeatureType featureType, Class c) throws IllegalAccessException, InstantiationException {
        if (! typedDatasetFactoryList.contains(c)) {
        	// try registering both types, one will throw exception.  this is a band-aid until full transition from TypedDataset... to FeatureDataset... is complete
        	try {
            	TypedDatasetFactory.registerFactory(featureType, c);
			} catch (Exception e) {
			}
        	try {
        		FeatureDatasetFactoryManager.registerFactory(featureType, c);
			} catch (Exception e) {
			}
...
//        }
        ...
        try {
            WCTIospManager.getInstance().registerTypedDatasetFactory(ucar.nc2.constants.FeatureType.RADIAL, gov.noaa.ncdc.wct.decoders.sigmet.SigmetDatasetFactory.class);
        } catch (Exception e) {
            e.printStackTrace();
