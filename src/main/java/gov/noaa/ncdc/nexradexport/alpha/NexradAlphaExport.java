...
public class NexradAlphaExport {
    ...
    public static void saveLineWKT(File outFile, DecodeL3Alpha decoder)
        ...
        if (decoder.getLineFeatures() != null) {
        	...
        	String basename = outFile.toString().endsWith(".txt") ? outFile.toString().substring(0, outFile.toString().length()-5) : outFile.toString();
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File(basename + "_line.txt")));
            try {
    ...
    public static void saveCSV(File outFile, DecodeL3Alpha decoder,
    	...
    	String ext = outFile.toString().endsWith(".csv") ? "" : ".csv";
        BufferedWriter bw = new BufferedWriter(new FileWriter(new File(outFile.toString()+ext)));
    ...
    public static void saveWKT(File outFile, DecodeL3Alpha decoder)
    	...
    	String ext = outFile.toString().endsWith(".txt") ? "" : ".txt";
        BufferedWriter bw = new BufferedWriter(new FileWriter(new File(outFile.toString()+ext)));
    ...
    public static void saveGeoJSON(File outFile, DecodeL3Alpha decoder)
            throws WCTExportException, WCTExportNoDataException,
            IOException, IllegalAttributeException {
    	...
    	String ext = outFile.toString().endsWith(".json") ? "" : ".json";
    	StreamingGeoJSONExport jsonExport = new StreamingGeoJSONExport(new File(outFile.toString()+ext));  
        try {        	      	
    ...
    public static void saveLineGeoJSON(File outFile, DecodeL3Alpha decoder)
        throws WCTExportException, WCTExportNoDataException,
            IOException, IllegalAttributeException {
        ...
        if (decoder.getLineFeatures() != null) {
        	...
        	String basename = outFile.toString().endsWith(".json") ? outFile.toString().substring(0, outFile.toString().length()-5) : outFile.toString();
        	
         	StreamingGeoJSONExport jsonExport = new StreamingGeoJSONExport(new File(basename+"_line.json"));  
            try {        	      	
            	
                FeatureCollection features = decoder.getLineFeatures();
                FeatureIterator fi = features.features();
                Feature f = null;
                int i = 0;
                while (fi.hasNext()) {
