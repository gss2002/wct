...
public class StackedBox extends JPanel implements Scrollable {
    ...
    private static final Icon CANCEL_ICON = new ImageIcon(WCTToolBar.class.getResource("/icons/dialog-cancel.png"));
    
    // from: http://www.iconarchive.com/show/oxygen-icons-by-oxygen-icons.org/Actions-document-edit-icon.html
    // license: GPL
    private static final Icon EDIT_ICON = new ImageIcon(WCTToolBar.class.getResource("/icons/edit.png"));
        ...
        if (isRemovable) {
            ...
            editButton.addActionListener(new ActionListener() {				
				...
				public void actionPerformed(ActionEvent evt) {
					Object newName = JOptionPane.showInputDialog(component, 
							"Current: "+link.getText(), "Input New Snapshot Layer Name", JOptionPane.QUESTION_MESSAGE,
							null, null, link.getText());
					if (newName != null && newName.toString().trim().length() != 0) {
						link.setText(newName.toString());						
					}
				}
			});
            
            JButton removeButton = new JButton(CANCEL_ICON);
