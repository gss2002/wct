...
public class WCTStatusBar extends JComponent implements MouseMotionListener {
    ...
    private GridCoordSystem gridCoords = null;
    ...
    /**
     * Added by Steve Ansari for WCT
     * @param gridCoordSys
     */
    public void setGridCoordSys(GridCoordSystem gridCoordSystem) {
    	this.gridCoords = gridCoordSystem;
    }
    ...
    private String getNexradCursorAzRanString(GeoMouseEvent event) {
        if (nexradHeader != null) {
        	...
        	if (gridCoords != null) {
        		coordpt = event.getMapCoordinate(coordpt);
        		gridCoords.findXYindexFromLatLon(coordpt.getY(), coordpt.getX(), xy);
        		return new StringBuilder().append("index=[").append(xy[0]).append(" , ").append(xy[1]).append("]").toString();
        	}
